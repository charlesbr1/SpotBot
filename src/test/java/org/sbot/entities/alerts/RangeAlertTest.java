...
public class RangeAlertTest {
    ...
    public static RangeAlert createTestRangeAlert() {
        return new RangeAlert(NEW_ALERT_ID, TEST_CLIENT_TYPE, TEST_USER_ID, TEST_SERVER_ID, TEST_FROM_DATE.minusMinutes(1L), TEST_FROM_DATE, TEST_EXCHANGE, TEST_PAIR, TEST_MESSAGE,
    ...
    void constructorCheck() {
        assertDoesNotThrow(() -> new RangeAlert(NEW_ALERT_ID, TEST_CLIENT_TYPE, TEST_USER_ID, TEST_SERVER_ID, TEST_FROM_DATE.minusMinutes(1L), TEST_FROM_DATE, TEST_EXCHANGE, TEST_PAIR, TEST_MESSAGE,
                TEST_FROM_PRICE, TEST_TO_PRICE, TEST_FROM_DATE, TEST_TO_DATE, TEST_LAST_TRIGGER,
                TEST_MARGIN, DEFAULT_REPEAT, DEFAULT_SNOOZE_HOURS));
        assertDoesNotThrow(() -> new RangeAlert(NEW_ALERT_ID, TEST_CLIENT_TYPE, TEST_USER_ID, TEST_SERVER_ID, TEST_FROM_DATE.minusMinutes(1L), null, TEST_EXCHANGE, TEST_PAIR, TEST_MESSAGE,
                TEST_FROM_PRICE, TEST_TO_PRICE, TEST_FROM_DATE, TEST_TO_DATE, TEST_LAST_TRIGGER,
                TEST_MARGIN, DEFAULT_REPEAT, DEFAULT_SNOOZE_HOURS));
        // clientType not null
        assertThrows(NullPointerException.class, () -> new RangeAlert(NEW_ALERT_ID, null, TEST_USER_ID, TEST_SERVER_ID, TEST_FROM_DATE.minusMinutes(1L), TEST_FROM_DATE, TEST_EXCHANGE, TEST_PAIR, TEST_MESSAGE,
                TEST_FROM_PRICE, TEST_TO_PRICE, TEST_FROM_DATE, TEST_TO_DATE, TEST_LAST_TRIGGER,
                TEST_MARGIN, DEFAULT_REPEAT, DEFAULT_SNOOZE_HOURS));
        // prices not null
        assertThrows(NullPointerException.class, () -> new RangeAlert(NEW_ALERT_ID, TEST_CLIENT_TYPE, TEST_USER_ID, TEST_SERVER_ID, null, TEST_FROM_DATE, TEST_EXCHANGE, TEST_PAIR, TEST_MESSAGE,
                TEST_FROM_PRICE, TEST_TO_PRICE, TEST_FROM_DATE, TEST_TO_DATE, TEST_LAST_TRIGGER,
                TEST_MARGIN, DEFAULT_REPEAT, DEFAULT_SNOOZE_HOURS));
        assertThrows(NullPointerException.class, () -> new RangeAlert(NEW_ALERT_ID, TEST_CLIENT_TYPE, TEST_USER_ID, TEST_SERVER_ID, TEST_FROM_DATE.minusMinutes(1L), TEST_FROM_DATE, TEST_EXCHANGE, TEST_PAIR, TEST_MESSAGE,
        ...
        assertThrows(IllegalArgumentException.class, () -> new RangeAlert(NEW_ALERT_ID, TEST_CLIENT_TYPE, TEST_USER_ID, TEST_SERVER_ID, TEST_FROM_DATE.minusMinutes(1L), TEST_FROM_DATE, TEST_EXCHANGE, TEST_PAIR, TEST_MESSAGE,
    ...
    void build() {
        ...
        assertNotNull(alert.build(NEW_ALERT_ID, TEST_CLIENT_TYPE, TEST_USER_ID, TEST_SERVER_ID, TEST_FROM_DATE.minusMinutes(1L), TEST_FROM_DATE, TEST_EXCHANGE, TEST_PAIR, TEST_MESSAGE,
                TEST_FROM_PRICE, TEST_TO_PRICE, TEST_FROM_DATE, TEST_TO_DATE, null,
                TEST_MARGIN, DEFAULT_REPEAT, DEFAULT_SNOOZE_HOURS));

        assertThrows(NullPointerException.class, () -> alert.build(NEW_ALERT_ID, TEST_CLIENT_TYPE, TEST_USER_ID, TEST_SERVER_ID, TEST_FROM_DATE.minusMinutes(1L), TEST_FROM_DATE, TEST_EXCHANGE, TEST_PAIR, TEST_MESSAGE,
    ...
    void asMessage() {
        ...
        assertFalse(message.contains("SNOOZE until"));
        assertFalse(alert.withListeningDateRepeat(now(), (short) 2).asMessage(MATCHED, previousClose, now).getDescriptionBuilder().toString().contains(DISABLED));
        assertFalse(alert.withListeningDateRepeat(now(), (short) 2).asMessage(MATCHED, previousClose, now).getDescriptionBuilder().toString().contains("SNOOZE until"));
        // with dates
        ...
        // with candlestick
        assertNotEquals(message, alert.asMessage(MARGIN, previousClose, now).getDescriptionBuilder().toString());
        ...
        assertFalse(message.contains("SNOOZE until"));
        assertFalse(alert.withListeningDateRepeat(now(), (short) 2).asMessage(MARGIN, previousClose, now).getDescriptionBuilder().toString().contains(DISABLED));
        assertFalse(alert.withListeningDateRepeat(now(), (short) 2).asMessage(MARGIN, previousClose, now).getDescriptionBuilder().toString().contains("SNOOZE until"));
        // with dates
        assertTrue(embed.getFields().stream().map(Field::getName).anyMatch("from date"::equals));
        ...
        assertTrue(alert.withListeningDateRepeat(now.plusHours(1L).plusMinutes(1L).plusSeconds(1L), (short) 1).asMessage(NOT_MATCHING, previousClose, now).getDescriptionBuilder().toString().contains("" + now.plusHours(1L).plusMinutes(1L).plusSeconds(1L).toEpochSecond()));
        assertTrue(alert.withListeningDateRepeat(now.plusHours(1L).plusMinutes(59L).plusSeconds(1L), (short) 1).asMessage(NOT_MATCHING, previousClose, now).getDescriptionBuilder().toString().contains("" + now.plusHours(1L).plusMinutes(59L).plusSeconds(1L).toEpochSecond()));
        assertTrue(alert.withListeningDateRepeat(now.plusHours(3L).plusSeconds(1L), (short) 1).asMessage(NOT_MATCHING, previousClose, now).getDescriptionBuilder().toString().contains("" + now.plusHours(3L).plusSeconds(1L).toEpochSecond()));
        // with dates
        assertTrue(embed.getFields().stream().map(Field::getName).anyMatch("from date"::equals));
        assertTrue(alert.withFromDate(null).asMessage(MARGIN, null, now)
                .getFields().stream().map(Field::getName).noneMatch("from date"::equals));
